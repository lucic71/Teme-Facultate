...
% conversației.
match_rule(Tokens, _UserMemory, rule(Expression, _, _, _, _)) :-
    intersection(Tokens, Expression, CommonWords),
    dif(CommonWords, []).
...
find_matching_rules(Tokens, Rules, UserMemory, MatchingRules) :-
    include(match_rule(Tokens, UserMemory), Rules, MatchingRules).
